...
def verify_semantic_eval(ac: dict) -> ACResult:
    """LLM 판정이 필요한 AC는 SKIP으로 반환한다.
    orchestrator 에이전트가 evaluator-llm 서브에이전트를 별도 spawn해 처리한다."""
    return ACResult(
        ac_id=ac["id"],
        description=ac["description"],
        verdict="SKIP",
        evidence="LLM 판정 필요 — orchestrator가 evaluator-llm 서브에이전트로 처리",
        details={"deferred_to": "evaluator-llm subagent"},
    )
...
def evaluate_sprint(sprint_id: str) -> SprintEvalResult:
    ...
    # SKIP = semantic_eval 등 LLM 위임 항목. overall 판정에서 제외.
...
def main() -> None:
    ...
    if args.json:
        skipped_acs = [r.ac_id for r in result.ac_results if r.verdict == "SKIP"]
        payload = {
            "sprint_id": result.sprint_id,
            "overall": result.overall,
            "failed_acs": result.failed_acs,
            "skipped_acs": skipped_acs,
