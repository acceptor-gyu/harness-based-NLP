...
tools:
  ...
  - Bash
  - Agent
description: 스프린트 루프 조율자. 각 서브에이전트의 .md 파일을 읽어 general-purpose 에이전트에 주입하는 방식으로 실행. [refactoring-planner → refactoring-plan-validator → generator → 병렬 리뷰 → review-synthesis →  evaluator.py → documenter] 순서로 조율. planner는 sprint-contract.yaml 없을 때만 실행.
---
...
- `.claude/agents/refactoring-plan-validator.md`
...
1. `.harness/refactoring-contract.yaml` 존재 여부 확인
   - 없으면 → **Step P: Refactoring Planner 실행** 후 계속
   - 있으면 → 그대로 사용
2. `.harness/claude-progress.txt` 읽기 — 현재/대기 리팩토링 스프린트 및 재시도 이력 파악
3. `.harness/refactoring-contract.yaml` 읽기 — 리팩토링 스프린트 목록 및 AC 목록 확인
...
sprint-06까지 모두 PASS 시 아래 요약 출력 후 종료:

```
=== 프로젝트 완료 ===
완료 스프린트: sprint-01 ~ sprint-06
총 시도 횟수: N회
최종 상태: PASS
```
